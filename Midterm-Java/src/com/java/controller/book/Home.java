...
public class Home extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (session.getAttribute("name") == null) {
			...
			ArrayList<PublisherModel> arrPublisher=new ArrayList<PublisherModel>();
			ResultSet rs = conn.getData(sql1);
			ResultSet rs2=conn.getData(sql2);
			ResultSet rs3=conn.getData(sql3);
				try {
			...
			req.setAttribute("publisher", arrPublisher.size()+"");
